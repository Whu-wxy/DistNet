...
def default_evaluation_params(flag):
    """
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    print('TIoU-metric:')
    print("tiouRecall:", round(methodRecall_tiouGt, 3), "tiouPrecision:", round(methodPrecision_tiouDt, 3),
          "tiouHmean:", round(tiouMethodHmean, 3))
...
def curve_cal_recall_precison_f1(type, gt_path, result_path, show_result=False):
    p = {'f': type, 'g': gt_path, 's': result_path}
    result = main_evaluation(p, default_evaluation_params, validate_data, evaluate_method,
                             show_result)
