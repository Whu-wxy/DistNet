...
from . import rrc_evaluation_funcs_13
import importlib
...
def validate_data(gtFilePath, submFilePath,evaluationParams):
    ...
    """
    gt = rrc_evaluation_funcs_13.load_zip_file(gtFilePath, evaluationParams['GT_SAMPLE_NAME_2_ID'])
    
    subm = rrc_evaluation_funcs_13.load_zip_file(submFilePath, evaluationParams['DET_SAMPLE_NAME_2_ID'], True)

    #Validate format of GroundTruth
    for k in gt:
        rrc_evaluation_funcs_13.validate_lines_in_file(k,gt[k],evaluationParams['CRLF'],True,True)
    ...
    for k in subm:
        ...
        rrc_evaluation_funcs_13.validate_lines_in_file(k,subm[k],evaluationParams['CRLF'],True,False)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        gtFile = rrc_evaluation_funcs_13.decode_utf8(gt[resFile])
        recall = 0
        precision = 0
        hmean = 0        
        recallAccum = 0.
        ...
        pointsList,_,transcriptionsList = rrc_evaluation_funcs_13.get_tl_line_values_from_file_contents(gtFile,evaluationParams['CRLF'],True,True,False)
        ...
        if resFile in subm:
            detFile = rrc_evaluation_funcs_13.decode_utf8(subm[resFile])
            pointsList,_,_ = rrc_evaluation_funcs_13.get_tl_line_values_from_file_contents(detFile,evaluationParams['CRLF'],True,False,False)
            for n in range(len(pointsList)):
                points = pointsList[n]            
...
def cal_recall_precison_f1_13(gt_path, result_path, show_result=False):
    p = {'g': gt_path, 's': result_path}
    result = rrc_evaluation_funcs_13.main_evaluation(p, default_evaluation_params, validate_data, evaluate_method,
                                                  show_result)
