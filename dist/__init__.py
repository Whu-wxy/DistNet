...
if subprocess.call(['make', '-C', BASE_DIR]) != 0:  # return value
    raise RuntimeError('Cannot compile dist: {}'.format(BASE_DIR))
...
def dist_warpper(region, center, probs=None):
    ...
    pred = dist_cpp(center, region, probs, 0.95, 0.95)
...
def decode(preds, scale, threshold=config.decode_threld):  # origin=0.7311
    ...
    pred, label_values = dist_warpper(region, center, preds)   #概率图改为传bi_region
