...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        gtFile = rrc_evaluation_funcs_deteval.decode_utf8(gt[resFile])
        recall = 0
        precision = 0
        hmean = 0        
        recallAccum = 0.
...
def cal_recall_precison_f1_deteval(gt_path, result_path, show_result=False):
    p = {'g': gt_path, 's': result_path}
    result = rrc_evaluation_funcs_deteval.main_evaluation(p, default_evaluation_params, validate_data, evaluate_method,
                                                  show_result)
