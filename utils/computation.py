...
def print_forward(model, inputs):
...
def show_summary(model, input_shape=3, save_path = ''):
    ...
    # Test on alexnet
    df = torch_summarize_df(input_size=(input_shape[0], input_shape[1], input_shape[2]), model=model)
    print(df)
    if save_path != '':
