# code from https://github.com/xingyizhou/CenterNet/blob/master/src/lib/models/networks/pose_dla_dcn.py
...
class DeformConv(nn.Module):
	def __init__(self, chi, cho):
		...
		self.offset_conv = nn.Conv2d(chi, 2*3*3, kernel_size=3, stride=1, padding=1, bias=False)
		self.conv = DCN(chi, cho, kernel_size=3, stride=1, padding=1, bias=False, dilation=1, groups=1)
...
class IDAUp(nn.Module):
	...
	def __init__(self, o, channels, up_f, bFSM=False):
...
class DLAUp(nn.Module):
	def __init__(self, startp, channels, scales, in_channels=None, bFSM=False):
...
class DLASeg(nn.Module):
	def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
				 last_level, head_conv, out_channel=0, bFSM=False):
		...
		self.ida_up = IDAUp(out_channel, channels[self.first_level:self.last_level],
							[2 ** i for i in range(self.last_level - self.first_level)], bFSM=bFSM)
		self.heads = heads
...
def get_dlaseg_net(num_layers, heads, head_conv=256, down_ratio=4, bFSM=False):
